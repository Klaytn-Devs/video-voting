    ...
    <>
      ...
      {!isLoadingFetchList && data && data.length > 0 && (
        <>
          ...
          <div className="z-10 mt-5 font-bold">TOTAL DEPOSITED: {dataTotalVote} VEV</div>
        </>
      )}
      <div className="z-10 my-5 grid w-full max-w-screen-xl grid-cols-1 gap-5 px-5 md:grid-cols-2 xl:px-0">
        {data &&
          data.length > 0 &&
          data.map(({ name, url, description, amountVote }, index) => (
            <div className="flex flex-col overflow-hidden rounded-lg">
              <a
                ...
                className="relative bg-gray-500 bg-cover bg-local bg-center bg-no-repeat text-left bg-blend-multiply hover:bg-blend-soft-light dark:hover:bg-blend-darken"
                style={{ height: '300px' }}
              >
                <iframe
                  width="100%"
                  height="100%"
                  src={`https://www.youtube.com/embed/${url.split('v=')[1]}?controls=0`}
                ...
                <div className="absolute bottom-0 z-10 p-8 text-white">
                  <p className="mb-5 max-w-xl text-xs leading-tight tracking-tight">
                    {description}
                  </p>
